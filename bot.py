...
async def send_request_to_admins(context, request_id, user_id, username, name, phone, role):
    """Отправляет заявку всем админам"""
    ...
    for admin_id in ADMIN_IDS:
        try:
            kb = InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ Принять", callback_data=f"apr_{request_id}"),
                InlineKeyboardButton("❌ Отклонить", callback_data=f"rej_{request_id}")
            ]])
...
async def all_requests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for req in requests:
        ...
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("✅ Принять", callback_data=f"apr_{req[0]}"),
            InlineKeyboardButton("❌ Отклонить", callback_data=f"rej_{req[0]}")
        ]])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ОБРАБОТКА ЗАЯВОК (НОВЫЕ) ---
    elif d.startswith("apr_"):
        request_id = int(d.split("_")[1])
        request = cursor.execute("""
            SELECT user_id, name, phone, role FROM requests WHERE id = ?
        """, (request_id,)).fetchone()
        ...
        try:
            if role == "student":
                cursor.execute("INSERT INTO students (telegram_id, name, phone) VALUES (?, ?, ?)", (user_id, name, phone))
            else:
                cursor.execute("INSERT INTO parents (telegram_id, name, phone) VALUES (?, ?, ?)", (user_id, name, phone))
            
            cursor.execute("UPDATE requests SET status = 'approved' WHERE id = ?", (request_id,))
            conn.commit()
            
            await q.edit_message_text(f"✅ Заявка #{request_id} одобрена, {name} добавлен")
            await context.bot.send_message(user_id, f"✅ Администратор добавил тебя как **{role}**!\nНапиши /start")
        except Exception as e:
        ...
        if request:
            ...
            await context.bot.send_message(request[0], "❌ К сожалению, твоя заявка отклонена администратором.")

    # --- ОСТАЛЬНЫЕ ФУНКЦИИ (СОКРАЩЕНО ДЛЯ КРАТКОСТИ, НО ОНИ ЕСТЬ В ПОЛНОМ КОДЕ) ---
    # Здесь идут остальные обработчики: add_student, add_parent, add_membership, freeze_menu и т.д.
    # В полной версии кода они все присутствуют, я сократил для ответа, но в твоём файле они остаются
...
async def request_phone(update, context):
    ...
    logger.info(f"📩 Пользователь: {username} (ID: {uid})")
    logger.info(f"📩 Имя: {name}")
    logger.info(f"📩 Телефон: {phone}")
    logger.info(f"📩 Роль: {role_text}")
    logger.info(f"👑 Админы для отправки: {ADMIN_IDS}")
    ...
    for admin_id in ADMIN_IDS:
        try:
            logger.info(f"📨 Попытка отправки админу {admin_id}...")
    ...
    if sent_count == 0:
        logger.error(f"⚠️ Заявка #{request_id} не доставлена ни одному админу!")
        await update.message.reply_text(
            "❌ Техническая ошибка. Ваша заявка сохранена, администратор получит уведомление позже."
        )
    else:
        await update.message.reply_text(
            f"✅ Заявка #{request_id} отправлена администратору. Ожидайте подтверждения."
        )
    
    context.user_data.clear()
    logger.info(f"📩 ===== ЗАЯВКА #{request_id} ОБРАБОТАНА =====\n")
...
def main():
    ...
    app.add_handler(CommandHandler("requests", all_requests))
    
    # Здесь идут все ConversationHandler'ы
    # В полной версии они все есть
    ...
    logger.info("🚀 Бот с исправленными заявками запущен")
    app.run_polling()
