...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ОДОБРЕНИЕ ЗАЯВОК ---
        ...
        await context.bot.send_message(user_id, "❌ К сожалению, твоя заявка отклонена администратором.")
...
async def request_phone(update, context):
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            kb = InlineKeyboardMarkup([[
                InlineKeyboardButton("✅ Принять", callback_data=f"approve_{role}_{uid}_{name}_{phone}_{request_id}"),
                InlineKeyboardButton("❌ Отклонить", callback_data=f"reject_{uid}_{request_id}")
            ]])
...
def main():
    ...
    logger.info("🚀 Бот с защитой от потери заявок запущен")
    app.run_polling()
