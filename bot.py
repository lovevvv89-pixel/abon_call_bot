...
cursor.execute('''CREATE TABLE IF NOT EXISTS memberships (id INTEGER PRIMARY KEY AUTOINCREMENT, student_id INTEGER, lessons_left INTEGER DEFAULT 0, valid_until TEXT, status TEXT DEFAULT 'active', FOREIGN KEY(student_id) REFERENCES students(id) ON DELETE CASCADE)''')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in ADMIN_IDS:
        kb = [
            ...
            [InlineKeyboardButton("📊 Отчёт по группе", callback_data="report_group")],
        ]
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("balance_"):
        ...
        if d == "admin_students":
            ...
            groups = cursor.execute("SELECT id, name FROM groups ORDER BY name").fetchall()
            if groups:
                kb = [[InlineKeyboardButton(f"📊 {g[1]}", callback_data=f"report_group_{g[0]}")] for g in groups]
                kb.append([InlineKeyboardButton("🔙 Назад", callback_data="start")])
                await q.edit_message_text("📊 Выберите группу для отчёта:", reply_markup=InlineKeyboardMarkup(kb))
            else:
                await q.edit_message_text("📚 Нет групп", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="start")]]))
        elif d.startswith("report_group_"):
            gid = int(d.split("_")[2])
            group = cursor.execute("SELECT name FROM groups WHERE id = ?", (gid,)).fetchone()
            students = cursor.execute("""
                SELECT s.id, s.name, 
                       COALESCE(m.lessons_left, 0) as left,
                       COALESCE((SELECT COUNT(*) FROM attendance WHERE student_id = s.id AND present = 1), 0) as visits,
                       COALESCE((SELECT COUNT(*) FROM attendance WHERE student_id = s.id AND present = 0), 0) as misses,
                       CASE 
                           WHEN COALESCE(m.lessons_left, 0) < 0 THEN ABS(COALESCE(m.lessons_left, 0))
                           ELSE 0 
                       END as debt
                FROM students s
            ...
            if not students:
                await q.edit_message_text(f"📊 {group[0]}\n\nНет учеников в группе", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="report_group")]]))
                return
            
            txt = f"📊 Отчёт по группе: {group[0]}\n{datetime.now().strftime('%d.%m.%Y')}\n\n"
            for s in students:
                txt += f"👤 {s[1]}\n"
                txt += f"  ├ Остаток: {s[2]}\n"
                txt += f"  ├ Посещений: {s[3]}\n"
            ...
            await q.edit_message_text(txt, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад", callback_data="report_group")]]))
...
async def add_group_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.edit_message_text("✏️ Введите название группы:")
    return GROUP_NAME
...
def main():
    ...
    logger.info("🚀 Бот с отчётами и смайликами запущен")
    app.run_polling()
